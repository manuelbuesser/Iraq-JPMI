...
prices <- data %>%
    ...
    ungroup() %>%
    group_by(date) %>%            
    mutate(water_trucking = ifelse(is.na(water_trucking),                               # impute missing water trucking values
           ...
           SMEB_water = 3*water + 47/500*water_trucking,
    ...
    rename(Date = date, Governorate = governorate, District = district,                 # rename column names 
           "SMEB" = SMEB, "SMEB food" = SMEB_food,
           "SMEB NFI" = SMEB_nfi, "SMEB water" = SMEB_water,
           ...
           "Disinfectant solution (1 kg)" = disinfectant_solution,
...
indicators <- indicators %>%
    ...
    rename(Date = date,
           ...
           District = district,
           'Challenge in past 30 days: Issues with check points or other movement restrictions' = challenges.check_points,
           'Challenge in past 30 days: Shortages in demand' = challenges.demand,
           'Challenge in past 30 days: I do not know' = challenges.dnk,
           'Challenge in past 30 days: Issues with government regulations' = challenges.government_regulations,
           'Challenge in past 30 days: Issues with insecurity or instability in the area' = challenges.insecurity,
           'Challenge in past 30 days: Shortages in liquidity' = challenges.liquidity,
           'Challenge in past 30 days: None' = challenges.none,
           'Challenge in past 30 days: Other' = challenges.other,
           
           'Harmful supply route change in past 30 days' = supply_routes,
           
           'Shortage in past 30 days: Lentils'        = lentils_shortage,
           'Shortage in past 30 days: Rice'           = rice_shortage,
           'Shortage in past 30 days: Sugar'          = sugar_shortage,
           'Shortage in past 30 days: Salt'           = salt_shortage,
           'Shortage in past 30 days: Bulgur'         = bulgur_shortage,
           'Shortage in past 30 days: Vegetable oil'  = vegetable_oil_shortage,
           'Shortage in past 30 days: Wheat flour'    = wheat_flour_shortage,
           'Shortage in past 30 days: Bath soap'      = bath_soap_shortage,
           'Shortage in past 30 days: Toothbrush'     = toothbrush_shortage,
           'Shortage in past 30 days: Toothpaste'     = toothpaste_shortage,
           'Shortage in past 30 days: Shampoo'        = shampoo_shortage,
           'Shortage in past 30 days: Sanitary napkins' = sanitary_napkins_shortage,
           'Shortage in past 30 days: Garbage bags'   = plastic_garbage_bags_shortage,
           'Shortage in past 30 days: Detergent'      = detergent_shortage,
           'Shortage in past 30 days: Disinfectant solution' = disinfectant_solution_shortage,
           'Shortage in past 30 days: Bottled water'  = water_shortage,
           'Shortage in past 30 days: Water trucking' = water_trucking_shortage,
           
           'Imported: Lentils'        = lentils_imported,
           'Imported: Rice'           = rice_imported,
           'Imported: Sugar'          = sugar_imported,
           'Imported: Salt'           = salt_imported,
           'Imported: Bulgur'         = bulgur_imported,
           'Imported: Vegetable oil'  = vegetable_oil_imported,
           'Imported: Wheat flour'    = wheat_flour_imported,
           'Imported: Bath soap'      = bath_soap_imported,
           'Imported: Toothbrush'     = toothbrush_imported,
           'Imported: Toothpaste'     = toothpaste_imported,
           'Imported: Shampoo'        = shampoo_imported,
           'Imported: Sanitary napkins' = sanitary_napkins_imported,
           'Imported: Garbage bags'   = plastic_garbage_bags_imported,
           'Imported: Detergent'      = detergent_imported,
           'Imported: Disinfectant solution' = disinfectant_solution_imported,
           'Imported: Bottled water'  = water_imported
    )
...
cols_home <- c("rgb(238,88,89)", "rgb(247,172,172)", "rgb(88,88,90)",             # define colors palette for home plot
               "rgb(172,172,173)")

full_list <- data.frame(Item = indicator_list,                                    # create full indicator list
                        Group = "VII. Additional Indicators") %>% arrange(Item)
...
prices_changes <- prices_country_long %>%                                         # calculate bi-monthly/yearly changes of item prices
    ...
    group_by(Item) %>%
    mutate(change = percent(Price/lag(Price, order_by=Date)-1, accuracy = 0.1),
           change2 = percent(Price/lag(Price, n = 2, order_by=Date)-1, accuracy = 0.1)) %>%
...
table_changes_meb <- prices_changes_meb %>%                                                               # style key figures table
        ...
        column_spec(3, color = ifelse(prices_changes_meb$'Bi-monthly change' == "0.0%", "auto", ifelse(prices_changes_meb$'Bi-monthly change' > 0, "red", "green"))) %>%
        column_spec(4, color = ifelse(prices_changes_meb$'Yearly change' == "0.0%" | prices_changes_meb$'Yearly change' == "NA", "auto", ifelse(prices_changes_meb$'Yearly change' > 0, "red", "green")))
...
table_changes <- prices_changes_items %>%                                                                 # style item table
    ...
    column_spec(3, color = ifelse(prices_changes_items$'Bi-monthly change' == "0.0%", "auto", ifelse(prices_changes_items$'Bi-monthly change' > 0, "red", "green"))) %>%
    column_spec(4, color = ifelse(prices_changes_items$'Yearly change' == "0.0%" | prices_changes_items$'Yearly change' == "NA", "auto", ifelse(prices_changes_items$'Yearly change' > 0, "red", "green"))) %>%
...
ui <- bootstrapPage(
    ...
    navbarPage("Joint Price Monitoring Initiative (JPMI)",                        # define dashboard title
                        ...
                        div(class="dashboard",                                                                # set dashboard class from CSS file
                            ...
                            absolutePanel(id = "dropdown", top = 47, left = 560, width = 200, fixed=FALSE, draggable = FALSE, height = "auto",
                                          dropdown(
                                              h4("SMEB contents"),
                                              HTML(smeb_kbl),
                                              tags$i(h6("The Survival Minimum Expenditure Basket (SMEB) represents the minimum culturally adjusted group of items
                                                             required to support a six-person Iraqi household for one month, as defined by the CWG.")),
                                              width = "360px",
                                              tooltip = tooltipOptions(title = "Click to find more information about the composition of the SMEB."),
                                              size = "xs",
                                              up = FALSE,
                            ...
                            sidebarPanel(
                                ...
                                absolutePanel(id = "dropdown", bottom = 20, left = 20, width = 200,                            # define blue info button
                                              ...
                                              dropdown(
                                                  h4("SMEB contents"),
                                                  HTML(smeb_kbl),
                                                  tags$i(h6("The Survival Minimum Expenditure Basket (SMEB) represents the minimum culturally
                                                                 adjusted group of items required to support a six-person Iraqi household for one month,
                                                                 as defined by the CWG.")),
                                                  size = "xs",
                                                  up = TRUE,
                                                  style = "jelly",
                                                  icon = icon("info"), width = "360px",
                                                  tooltip = tooltipOptions(title = "Click to find more information about the composition of the SMEB."),
                                                  animate = animateOptions(
                                                      enter = "fadeInLeftBig",
                                                      exit = "fadeOutLeft",
                        ...
                        div(class="outer",
                            ...
                            absolutePanel(
                                ...
                                pickerInput("map_indicator_select",
                                            label = "Indicator:",   
                                            choices = c("Prices", "Additional indicators"),
                                            selected = "Prices",
                                            multiple = FALSE
                                ),
                                ...
                                conditionalPanel(condition = "input.map_indicator_select == 'Additional indicators'",
                                                 pickerInput("map_addindicator_select",
                                                             label = "Additional Indicators:",   
                                                             choices = sort(indicator_list),
                                                             selected = "Challenge in past 30 days: None",
                                                             options = list(`actions-box` = TRUE),
                                                             multiple = FALSE
                                                 )
                                ),
                            ...
                            conditionalPanel(condition = "input.map_indicator_select == 'Prices' & (input.map_item_select == 'SMEB' | input.map_item_select == 'SMEB food' | input.map_item_select == 'SMEB NFI' | input.map_item_select == 'SMEB water')",
                                             absolutePanel(id = "dropdown", bottom = 20, left = 20, fixed=FALSE, draggable = FALSE,
                                                           dropdown(
                                                               h4("SMEB contents"),
                                                               HTML(smeb_kbl),
                                                               tags$i(h6("The Survival Minimum Expenditure Basket (SMEB) represents the minimum culturally adjusted group of items
               ...
               tabPanel("Info", icon = icon("info"),
                        ...
                        column(
                            #h4("Last Update"),
                            #p(format(dates_max, "%B %Y"), " - This site is updated on a bi-monthly basis after every data collection round.",
                            #  style="text-align:justify;"),
                            #br(),
                            h4("Background"),
                        ...
                        column(
                            ...
                            tags$a(href="https://www.care-international.org/", "CARE"), "/", tags$a(href="https://harikar.org/", "Harikar"), br(),
                            ...
                            tags$a(href="https://www.clovekvtisni.cz/en/", "People in Need (PIN)"),
                            width=6)
               ),
               ...
               tabPanel("JRAM", icon = icon("map-marker"),
                        ...
                        div(class="outer",
                            ...
                            absolutePanel(
                                id = "controls", class = "panel panel-default", fixed = TRUE, draggable = TRUE, top = "130", left = "12", right = "auto", bottom = "auto",
                                width = 300, height = "auto",
...
server <- function(input, output, session) {
    ...
    observe({
        ...
        labels <- sprintf("<strong>%s</strong><br/>%s IQD (%s)", district$ADM2_EN, format(district[[map_indicator_select()]], big.mark=","), format(district$Date.x, "%b %Y")) %>% lapply(htmltools::HTML)
            ...
            labels <- sprintf("<strong>%s</strong><br/>%s&#37; (%s)", district$ADM2_EN, district[[map_indicator_select()]], format(district$Date.x, "%b %Y")) %>% lapply(htmltools::HTML)
        }
        ...
        leafletProxy("map") %>%
            ...
            execute_if(input$map_indicator_select == "Additional indicators",
